...
export async function getParamsForProof(address, blocknumber, rpcURL) {
  const sepoliaBaseProvider = new ethers.JsonRpcProvider(rpcURL);
  const block = await sepoliaBaseProvider.send("eth_getBlockByNumber", [
  ...
  const eth_getProofResult = await sepoliaBaseProvider.send("eth_getProof", [
    address,
