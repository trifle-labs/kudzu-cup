...
async function processAddressBatch(KudzuBurn, addresses, quantities, rewardIds, retryCount = 0) {
  ...
  try {
    ...
    if (retryCount < maxRetries) {
      ...
      return processAddressBatch(KudzuBurn, addresses, quantities, rewardIds, retryCount + 1);
    } else {
...
async function main() {
  ...
  if (updateBlockNum === 999999999999) {
    if (chainId === 984122) {
      ...
      console.error(`This is using a test block number ${updateBlockNum} on chain ${chainId}`);
    }
  }
  ...
  for (let i = 0; i < addresses.length; i += batchSize) {
    const batchAddresses = addresses.slice(i, i + batchSize).map((a) => a.address);
    const batchQuantities = addresses.slice(i, i + batchSize).map((a) => a.points);
    const batchRewardIds = addresses.slice(i, i + batchSize).map((a) => a.rewardId);
  ...
  if (failedBatches.length > 0) {
    console.log(`${failedBatches.length} batches failed and were saved to failed_batches.json`);
  }
}
...
//   const url =
//     'https://api.indexsupply.net/query?query=SELECT+%0A++t.from+as+%22burner%22%2C+t.value%0AFROM+%0A++transfersingle+as+t%0AWHERE%0A++address+%3D+0x18130De989d8883c18e0bdBBD3518b4ec1F28f7E%0AAND%0A++t.to+%3D+0x000000000000000000000000000000000000dEad%0AAND%0A++t.from+%21%3D+0x0000000000000000000000000000000000000000%0AAND%0A++t.block_num+%3C%3D+10211963%0AAND%0A++t.value+%3E+1%0A&event_signatures=TransferSingle%28address+indexed+operator%2C+address+indexed+from%2C+address+indexed+to%2C+uint256+id%2C+uint256+value%29&event_signatures=&chain=984122';
//   const response = await fetch(url);
//   const data = await response.json();
//   const addresses = data.result[0]
//     .slice(1) // Skip the header row
