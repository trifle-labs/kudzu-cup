...
describe('KudzuBurn Tests', function () {
  ...
  it('burn fails under various invalid conditions', async () => {
    ...
    await expect(KudzuBurn.connect(acct1).burn(nonExistentTokenId, 1)).to.be
      .reverted;

    // Try to burn when caller has no tokens
    await expect(KudzuBurn.connect(randomAccount).burn(tokenIds[0], 1)).to.be
      .reverted;
  ...
  it('reverts when all rounds are over', async () => {
    ...
    await expect(KudzuBurn.burn(tokenIds[0], 1)).to.be.revertedWith(
      'All rounds are over'
    );
  ...
  it('adminMassReward and adminMassPunish work correctly and emit events', async () => {
    ...
    for (let i = 0; i < massPunishEvents.length; i++) {
      ...
      if (i % 2 === 0) {
        ...
        expect(massPunishEvents[i].args.points).to.equal(
          -1 * excessivePunishQuantities[ii]
        );
      } else {
        const resultingBalance =
          newBalances[ii] - excessivePunishQuantities[ii];
