...
describe("ERC2981", function () {
  ...
  it("Should set default royalty recipient and amount", async function () {
    const royalty = await erc2981.royalty();
    expect(royalty.recipient).to.equal(owner.address);
    expect(royalty.amount).to.equal(1000);
  });

  it("Should allow owner to set token royalty", async function () {
  ...
  it("Should revert if non-owner tries to set token royalty", async function () {
    await expect(
      erc2981.connect(addr1).setTokenRoyalty(addr1.address, 500)
    ).to.be.revertedWith("Ownable: caller is not the owner");
  });

  it("Should revert if royalty value is greater than 10000", async function () {
    await expect(
      erc2981.setTokenRoyalty(addr1.address, 10001)
    ).to.be.revertedWith(
  ...
  it("Should support IERC2981 and IERC165 interfaces", async function () {
